...
namespace JWT
{
    ...
    public static class JsonWebToken
    {
        ...
        public static IJsonSerializer JsonSerializer
        {
            get
            {
                if(_jsonSerializer == null)
                    throw new NoJsonSerializerException();
        ...
        public static string Decode(string token, byte[] key, bool verify = true)
        {
            ...
            var headerArray = Base64UrlDecode(header);
            var payloadArray = Base64UrlDecode(payload);

            var headerJson = Encoding.UTF8.GetString(headerArray, 0, headerArray.Length);
            var payloadJson = Encoding.UTF8.GetString(payloadArray, 0, payloadArray.Length);
